...
function getNum(min, max, number) {
	if (number < max && number > min) {
		console.log(`Число ${number} находитя в диапазоне: ${min} - ${max}`)
	} else {
		console.log(`Число ${number} не находитя в диапазоне: ${min} - ${max}`)
...
for (let propety in engineers) {
	console.log(`Заработная плата ${propety} составляет ${engineers[propety]} рублей`);
}
...
for (i = 0; i < numbers.length; i++) {
	console.log(`Индексу ${i} соотвестувет число ${numbers[i]}`);
}
...
console.log(`Cумма чисел: ${getSum(numbers)}`);



const getEvenNum = (numbers) => {
	let result = 0;
	for (let i = 0; i < numbers.length; i++) {
		if (numbers[i] % 2 === 0) {
...
console.log(`Сумма чётных чисел: ${getEvenNum(numbers)}`);
...
console.log(`Индекс(ы) максимального(ых) числа(ел) массива: ${newArray}`);
...
let array = [23, 12, 311, 112, 12, 121, 98, 100]
let num = 0;
const getNewSecondArray = (array, num) => {
	let newSecondArray = [];
	for (i = 0; i < array.length; i++) {
		if (array[i] >= num) {
			newSecondArray.push(array[i]);
		}
	}
	return newSecondArray;
}
console.log(getNewSecondArray(array, 100));
...
for (i = 0; i < vegetables.length; i++) {
	wordLength = vegetables[i].length
	newVegetables = { word: vegetables[i], length: wordLength };

	newVegetablesArray.push(newVegetables);

}
console.log(newVegetablesArray);

for (i = 0; i < newVegetablesArray.length; i++) {
	console.log(`${newVegetablesArray[i].word} - ${newVegetablesArray[i].length}`)
}
